...
function sendBooking() {
  ...
  const entries = bookingForm.entries();
  ...
  Booking.form.reset();
  /* Reset wrapperTavoli click event */
  Booking.wrapperTavoli.querySelectorAll('.tavolo').forEach((tavolo) => {
    tavolo.classList.remove('selezionato');
  });

}

async function fetchDati(bookingForm) {
  let dati = await fetch('./booking/prenotazioni.php', {
    method: 'post',
    headers: {
